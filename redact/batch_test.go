...
func TestBatchBytesWithPrivacyFilter_MatchesPerBlobOutput(t *testing.T) {
	...
	// per-blob made exactly 3 batch calls — one per JSON-shaped blob
	// (full.jsonl, metadata.json, duplicate.jsonl). The .txt blob goes
	// through BytesWithPrivacyFilter → StringWithPrivacyFilter → the
	// singular Redact path, not RedactBatch, so it doesn't bump
	// batchCalls. Within a JSON blob, leaves are deduped, but across
	// blobs in the per-blob path they aren't — that's the inefficiency
	// the batched API addresses.
	if fake2.batchCalls != 1 {
		t.Errorf("want exactly 1 batched call, got %d", fake2.batchCalls)
	}
	if fake1.batchCalls != 3 {
		t.Errorf("want exactly 3 per-blob batch calls (one per JSON-shaped blob), got %d", fake1.batchCalls)
	}
}
